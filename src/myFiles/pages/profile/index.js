...
const Profile = function () {
  ...
  const createAvatar = async (file) => {
    try {
      ...
      data.append(
        "data",
        JSON.stringify({ users_permissions_user: userInfo.id })
      );
  ...
  const handleSubmit = async (e) => {
    ...
    if (avatar.length > 0) {
      ...
      if (avatar[0].attributes.avatar.data) {
        await axios.delete(
          `${api.baseUrl}/api/upload/files/${avatar[0].attributes.avatar.data.id}`,
          {
            headers: {
              Authorization: `Bearer ${auth.getToken()}`,
            },
  ...
  const getAvatar = async () => {
    try {
      const res = await axios.get(
        `${api.baseUrl}/api/user-avatars?populate=*&filters[users_permissions_user]=${userInfo.id}`
      );
        ...
        <div className="row">
          <div className="col-md-12">
            <div className="px-2 px-md-5">
              <div
                className="bg-success position-relative border-white border border-4 rounded-circle p-1 uploadDiv"
                style={{
                  width: "150px",
                  height: "150px",
                  marginTop: "-120px",
            ...
            <div className="px-2 px-md-5 d-flex  align-items-start justify-content-between flex-column flex-md-row flex-md-row-reverse">
              <div className="d-flex align-items-center justify-content-center collection-social-icon-parent ms-auto"></div>
              <div className="mt-4 d-flex align-items-center">
                <h2 className="fw-normal m-0 me-2 fs-2">
                  {/* {hotCollections && hotCollections.name} */}
                  {`${userInfo.address.slice(0, 5)}...${userInfo.address.slice(
                    37,
                    42
                  )}`}
                </h2>
                <i
                  className="fa fa-check text-white bg-primary p-1 rounded-5 "
      ...
      {showUpload && (
        <div className="checkout">
          <div className="maincheckout">
            <button className="btn-close" onClick={handleUploadClose}>
              x
            </button>
